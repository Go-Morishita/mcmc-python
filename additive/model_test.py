...
# 粒子位置
particles_positions = ti.Vector.field(
    2, dtype=ti.f32, shape=(num_chains, NUM_PARTICLES))
# 提案位置
proposal_positions = ti.Vector.field(
    2, dtype=ti.f32, shape=(num_chains, NUM_PARTICLES))
# 現在の確率
...
def cal_kappa2(r):
    # REJECT_POINT以下は0にする
    return ti.select(r < REJECT_POINT, -1.0, SCALING_FACTOR * kappa2(r) + GETA)
...
def metrpolis_hastings_onestep(step_idx: ti.i32):  # type: ignore[valid-type]
    for chain in range(num_chains):
        # 提案位置の生成
        # ti.staticはコンパイラに対して、ループの長さが固定であることを伝える
        for particle in ti.static(range(NUM_PARTICLES)):
            delta = ti.randn() * PROPOSAL_STD
            position = particles_positions[chain, particle] + \
                ti.Vector([delta, ti.randn()*PROPOSAL_STD])
            proposal_positions[chain, particle] = (
                position + SIDE) % SIDE  # 周期境界条件を考慮して位置を更新する
        ...
        if ti.random() < proposal_probability / current_probability[chain]:
            for particle in ti.static(range(NUM_PARTICLES)):
                particles_positions[chain,
                                    particle] = proposal_positions[chain, particle]
        ...
        if step_idx % STORE_INTERVAL == 0:
            ...
            dist = ti.abs(
                (particles_positions[chain, 0] - particles_positions[chain, 1]))
...
axes[1].bar(bin_centers, nomalized_samples, width=bin_edges[1] -
            bin_edges[0], alpha=0.6, label='Empirical Density')
